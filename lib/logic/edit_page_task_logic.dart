...
//    });
  ...
  void pickEndTime(GlobalModel globalModel) {
    ...
    showDP(firstDate, initialDate, lastDate,
            globalModel.logic?.isDarkNow() ?? false)
        .then(
      (day) {
        if (day == null) return;
            ...
            showDialog(
                ...
                builder: (ctx) {
                  return AlertDialog(
                    ...
                    content:
                        Text(IntlLocalizations.of(ctx)?.endBeforeStart ?? ""),
                  );
                });
            return;
          }
                ...
                builder: (ctx) {
                  return AlertDialog(
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.all(Radius.circular(20.0))),
                    content:
                        Text(IntlLocalizations.of(ctx)?.startAfterEnd ?? ""),
                  );
                });
            return;
          }
